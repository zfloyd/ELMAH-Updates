...
namespace Elmah
{
	...
	internal sealed class ErrorLogPage : ErrorPageBase
	{
		...
		protected override void OnLoad(EventArgs e)
		{
			if (CanManageErrors())
			{
				foreach (string formField in Request.QueryString.Keys)
				{
					if (Request.QueryString[formField] == "on" && formField != "filter")
						this.ErrorLog.Delete(new Guid(formField));
				}
			}
			//
			...
			_totalCount = this.ErrorLog.GetErrors(_pageIndex, _pageSize, filter, _errorEntryList);
		...
		private void RenderSearchBox(HtmlTextWriter writer)
		{
			Debug.Assert(writer != null);

			TextBox uxSearchBox = new TextBox();
			uxSearchBox.Attributes.Add("placeholder", "Enter Search Text");
			uxSearchBox.Width = 300;
			uxSearchBox.Style.Add("margin-left", "50px");
			uxSearchBox.ID = "filter";
			uxSearchBox.Text = Request.QueryString["filter"];

			Button uxSearchButton = new Button();
			uxSearchButton.Text = "Search";

			uxSearchBox.RenderControl(writer);
			uxSearchButton.RenderControl(writer);
		}
		...
		private void RenderErrors(HtmlTextWriter writer)
		{
			...
			if (CanManageErrors())
				headRow.Cells.Add(FormatCell(new TableHeaderCell(), "Delete", "delete-col"));
			...
			for (int errorIndex = 0; errorIndex < _errorEntryList.Count; errorIndex++)
			{
				...
				if (CanManageErrors())
					bodyRow.Cells.Add(FormatCellWithCheckbox(new TableCell(), errorEntry.Id));
	...
	for(var i = 0; i < checkBoxes.length; i++)
	{
		...
		private TableCell FormatCellWithCheckbox(TableCell cell, string errorID)
		{
			Debug.Assert(cell != null);

			CheckBox uxDelete = new CheckBox();
			uxDelete.ID = errorID;
			uxDelete.InputAttributes.Add("style", "margin: 0 14px;");
			cell.Controls.Add(uxDelete);
		...
		private bool CanManageErrors()
		{
			return !SecurityConfiguration.Default.RestrictManagement || (Request.ServerVariables["HTTP_GLID"] ?? "") == "starmidd";
		}
	}
}
