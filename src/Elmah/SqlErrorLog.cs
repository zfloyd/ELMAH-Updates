...
namespace Elmah
{
	...
	public class SqlErrorLog : ErrorLog
	{
		...
		internal override void Delete(Guid id)
		{
			...
			using (SqlConnection connection = new SqlConnection(this.ConnectionString))
			using (SqlCommand command = Commands.DeleteError(
				id))
			{
				command.Connection = connection;
				connection.Open();
		...
		{
			return GetErrors(pageIndex, pageSize, "", errorEntryList);
		}
		...
		public override int GetErrors(int pageIndex, int pageSize, string filter, IList errorEntryList)
		{
			...
			using (SqlConnection connection = new SqlConnection(this.ConnectionString))
			using (SqlCommand command = Commands.GetErrorsXml(this.ApplicationName, pageIndex, pageSize, filter))
			{
				command.Connection = connection;
				connection.Open();
		...
		private sealed class Commands
		{
			...
			public static SqlCommand GetErrorsXml(string appName, int pageIndex, int pageSize, string filter)
			{
				...
				parameters.Add("@Filter", SqlDbType.VarChar, 2000).Value = filter;
			...
			public static SqlCommand DeleteError(
				Guid id)
			{
				SqlCommand command = new SqlCommand("ELMAH_DeleteError");
				command.CommandType = CommandType.StoredProcedure;
